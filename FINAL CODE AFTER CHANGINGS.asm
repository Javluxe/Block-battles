...
.data
    ...
    ; computer block position save variables
    cPos1       db 0
    cPos2       db 0
...
computerTurn PROC
    ...
    ; keep trying random positions until we find one not fired
    ct_random:
        ; read system timer
        mov ah, 00h
        ...
        mov al, dl
        add al, cRow        ; add turn counter for variety
        mov ah, 0           ; clear ah to prevent overflow
        mov bl, 25
        div bl              ; ah = remainder (0-24)
        mov cIdx, ah        ; save as our random position
        ...
        cmp al, 2           ; already hit?
        je  ct_random       ; yes, try again
        cmp al, 3           ; already miss?
        je  ct_random       ; yes, try again

        ; fire here! hit or miss?
        cmp al, 1
        jne ct_cmiss
        ...
        mov byte ptr [si+bx], 2
        inc compHits
        inc cRow            ; increment turn counter
        mov drawWho, 0
        call drawGameGrid
        SET_CURSOR 20, 0
        mov ah, 09h
...
setupComputer PROC
    ...
    mov ah, 00h
    int 1Ah             ; CX:DX = tick count
    mov al, dl          ; low byte of DX only
    mov ah, 0           ; prevent overflow
    mov bl, 25
    div bl              ; ah = remainder 0-24
    mov bl, ah
    mov bh, 0
    mov cPos1, bl       ; save to memory!
    mov byte ptr [si+bx], 1
    ...
    mov ah, 00h
    int 1Ah
    mov al, dh          ; high byte of DX for variety
    mov ah, 0
    mov bl, 25
    div bl
    mov bl, ah
    ...
    cmp bl, cPos1       ; duplicate check with block 1
    je  sc_block2

    mov cPos2, bl       ; save to memory!
    mov byte ptr [si+bx], 1
    ...
    int 1Ah
    mov al, dl
    add al, 17          ; add offset for variety
    mov ah, 0
    mov bl, 25
    div bl
    mov bl, ah
    ...
    cmp bl, cPos1       ; duplicate check with block 1
    je  sc_block3
    cmp bl, cPos2       ; duplicate check with block 2
    je  sc_block3
